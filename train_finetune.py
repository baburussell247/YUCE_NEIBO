...
def get_vis_data(
    ...
    vis_data = {"input": (data[:, 0])}
    np.save(osp.join(output_dir, "input_data.npy"), vis_data)
    ...
    for t in range(num_timestamps):
        ...
        for i in range(data_t.shape[0]):
            # wind_data_t.append(data_t[i][0])
            wind_data_t.append(data_t[i])
        vis_data[f"target_{hour}h"] = np.asarray(wind_data_t)
        wind_data.append(wind_data_t)
    wind_data = np.asarray(wind_data)

    # Save wind_data and vis_data to files
    np.save(osp.join(output_dir, "true_data.npy"), wind_data)
...
def evaluate(cfg: DictConfig):
    ...
    # set visualizer data
    DATE_STRINGS = ("2024-01-01 00:00:00",)
    ...
    # Define a global variable to store predictions
    predictions = []
    ...
    def output_wind_func(d, var_name, data_mean, data_std):
        ...
        for i in range(output.shape[0]):
            ...
            wind_data.append(output[i])
        predictions.append(wind_data)
        np.save(osp.join(cfg.output_dir, "predictions.npy"), np.array(predictions))
    ...
    visualizer = {
        "visualize_wind": ppsci.visualize.VisualizerWeather(
            vis_data,
            vis_output_expr,
            xticks=np.linspace(0, 1255, 6),
            xticklabels=[str(i) for i in range(115, 121)],
            yticks=np.linspace(0, 335, 4),
            yticklabels=[str(i) for i in range(19, 23)],
            vmin=16,
            vmax=26,
            colorbar_label="°",
            batch_size=cfg.EVAL.batch_size,
